...
class syscoonFinanceinterfaceBookingform(models.TransientModel):
    ...
    def _compute_account(self):
        if self.account:
            partner = False
            account_id = self.search_account_id(self.account)
            if not account_id:
                ...
                #if not partner and self.move_type and self.move_type == 'customer_invoice':
                #    raise UserError(_('You have set "Customer Invoice (A)" for your move, but the account you have choosen is not linked to a customer'))
                if partner:
                    self.partner_id = partner.id

    @api.multi
    @api.onchange('counteraccount')
    def _compute_counteraccount(self):
        if self.counteraccount:
            partner = False
            account_id = self.search_account_id(self.counteraccount)
            if not account_id:
                raise UserError(_('Counteraccount does not exist!'))
            else:
                if not self.partner_id:
                    partner = self.get_partner(account_id)
                self.counteraccount_id = account_id.id
                #if not partner and self.move_type and self.move_type == 'supplier_invoice':
                #    raise UserError(_('You have set "Supplier Invoice (E)" for your move, but the counteraccount you have choosen is not linked to a supplier'))
                if partner:
                    self.partner_id = partner.id
    ...
    counteraccount = fields.Char('Counteraccount')
    ...
    account_id = fields.Many2one('account.account', compute='_compute_account', string='Account', readonly=1)
    counteraccount_id = fields.Many2one('account.account', compute='_compute_counteraccount', string='Counteraccount', readonly=1)
