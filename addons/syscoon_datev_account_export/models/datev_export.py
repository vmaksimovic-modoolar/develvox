...
class DatevExport(models.TransientModel):
    ...
    state = fields.Selection([('choose', 'choose'),
                              ('get', 'get')])
    filter = fields.Selection([('all', 'Alle'),
                               ('new', 'Nur neue')], "Welche wollen Sie exportieren?")
    ...
    def generate_header(self):
        ...
        buchung.append((ustr(current_date.year) + '0101'))  # WJ-Beginn
    ...
    def get_users(self, this):
        ...
        if this.filter == 'all':
            ...
            partners = partner_obj.search([('DatevExported', '=', False), ('parent_id', '=', False)])
    ...
    def generate_person_lines(self, this, partner):
        ...
        if this.filter == 'new':
            if partner.customer:
                if partner.property_account_receivable_id.user_type_id.type.lower() == 'receivable' and not partner.property_account_receivable_id.DatevExported:
                    lines.append(self.generate_columns(partner, partner.property_account_receivable_id.code))
                    account_ids.append(partner.property_account_receivable_id.id)

            if partner.supplier:
                if partner.property_account_payable_id.user_type_id.type.lower() == 'payable' and not partner.property_account_payable_id.DatevExported:
                    lines.append(self.generate_columns(partner, partner.property_account_payable_id.code))
                    account_ids.append(partner.property_account_payable_id.id)
    ...
    def generate_csv(self, this):
        ...
        self.env['account.account'].search([('id', 'in', account_ids)]).write({'DatevExported': True})
        # partners.write({'DatevExported': True})
