...
class ecofi(models.Model):
    ...
    def field_config(self, move, line, errorcount, partnererror, thislog, thismovename, faelligkeit, datevdict, belegdatum_format=False):
        ...
        if line.journal_id.type != 'sale' or line.journal_id.type != 'purchase':
            if line.payment_id.invoice_ids:
                datevdict['Beleg1'] = line.payment_id.invoice_ids[0].number
                datevdict['Beleg1'] = ustr(datevdict['Beleg1'].replace('/', ''))
        ...
        if line.move_id.ecofi_buchungstext:
            line_name.append(str(line.move_id.ecofi_buchungstext))
        elif line_name[0] == 'Buchung':
    ...
    def generate_csv(self, ecofi_csv, bookingdict, log):
        ...
        if berater_nr and mandanten_nr:
            ...
            datum_von = '%s%s%s' % (datum_von_str[4:8], datum_von_str[2:4], datum_von_str[0:2])
            datum_bis = '%s%s%s' % (datum_bis_str[4:8], datum_bis_str[2:4], datum_bis_str[0:2])
            ...
            automatisierungs_header_dict = {
                ...
                'WJBeginn': int(str(datum_von[6:10]) + '0101'),
                'Sachkontenlaenge': 4,
                'Datumvon': int(datum_von),
                'Datumbis': int(datum_bis),
                'Bezeichnung': '',
                'Diktatkuerzel': '',
                'Buchungstyp': '',
    ...
    def generate_csv_move_lines(self, move, buchungserror, errorcount, thislog, thismovename,
        ...
        if self._context.get('export_interface', '') == 'datev':
            ...
            for line in move.line_ids:
                ...
                datevgegenkonto = line.account_id.code
                ...
                (errorcount, partnererror, thislog, thismovename, datevdict) = \
                    ...
                    thismovename, faelligkeit, datevline, belegdatum_format)

                if not any(gl['Konto'] == datevkonto and gl['Gegenkonto'] == datevgegenkonto and gl['Buschluessel'] == buschluessel and gl['Sollhaben'] == sollhaben for gl in grouped_lines):
                    grouped_lines.append(datevline)
                else:
                    for gl in grouped_lines:
                        if gl['Konto'] == datevkonto and gl['Gegenkonto'] == datevgegenkonto and gl['Buschluessel'] == buschluessel and gl['Sollhaben'] == sollhaben:
                            gl['Umsatz'] = str(float(gl['Umsatz'].replace(',', '.')) + float(umsatz.replace(',', '.'))).replace('.', ',')
                            break

            for datevdict in grouped_lines:
                umsatz = datevdict['Umsatz'].replace(',', '.')
                umsatz_neu = Decimal(float(umsatz)).quantize(Decimal('.01'), rounding=ROUND_HALF_DOWN)
                datevdict['Umsatz'] = str(umsatz_neu).replace('.', ',')
